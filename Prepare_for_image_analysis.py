...
PATH_TO_IMAGES = '/Volumes/krauss/Lisa/GMR/Array/250206/Experiment_1_ethanol/loc1_1/20250206'
IMAGE_TYPE = 'tif'
...
SERVER_ROOT_PATH = '/home/chris/mnt/storage/Lisa/GMR/Array/250206/Experiment_1_ethanol/loc1_1'
...
def move_files(files_src_dest_name):
    ...
    for src_path, dest_path, name in files_src_dest_name:
        try:
            ...
            message.append(f"Successfully copied '{Path(src_path, name)}' to '{Path(dest_path, name)}'.")
        except PermissionError:
            message.append(f"Error: Permission denied while copying '{Path(src_path, name)}' to '{Path(dest_path, name)}'.")
            success = False
        except shutil.SameFileError:
            message.append(f"Error: Source and destination are the same file: '{Path(src_path, name)}'.")
            success = False
        except Exception as e:
            message.append(f"Error: Failed to copy '{Path(src_path, name)}' to '{Path(dest_path, name)}': {e}")
            success = False
    print('\n')
    print('\n'.join(message))
    return success
...
if skip != 'y':
    ...
    print("        NB: Filepaths should reflect the location from where the analysis script will be ran")
    ...
    if proceed != 'y':
        ...
        UOY_historical_image_list(PATH_TO_IMAGES, IMAGE_TYPE,
                                  'XXXXX', 'XXXXX', 'XXXXX',
...
generate_ROI_JSON(PATH_TO_IMAGES,
                  Path('Generated_files', f'{IMAGE_METADATA_NAME}.json'),
...
save_plots_of_ROI_locations(ROOT_PATH, path_to_first_img,
                            Path('Generated_files', f'{ROI_METADATA_NAME}.json'),
...
if 's' in location:
    image_metadata_server_path = Path(SERVER_ROOT_PATH, Path(PATH_TO_IMAGES).relative_to(ROOT_PATH))
...
if move == 'y':
    files_src_dest_name = [(src_path, PATH_TO_IMAGES, f'{IMAGE_METADATA_NAME}.json'),
...
backup = input("\nStep 10: Do you want to back-up all generated files to experiment directory? (y/n): ").lower()
if backup == 'y':
