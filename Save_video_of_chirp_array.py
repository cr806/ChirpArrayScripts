...
img_path = Path(img_dir, file_path).as_posix()

raw_im = cv.imread(img_path, cv.IMREAD_UNCHANGED)
im = cv.normalize(
    raw_im, None, 0, 255, cv.NORM_MINMAX, dtype=cv.CV_8U # type: ignore[arg-type]
...
for k, v in ROIs.items():
    ...
    cv.putText(im_color, v['label'], (x, y + 30), cv.FONT_HERSHEY_SIMPLEX, 1, color, 1)
...
with writer.saving(fig, video_filepath, dpi=100):
    for idx in range(starting_img_num, ending_img_num + 1):
        ...
        temp: np.ndarray = cv.normalize(
            temp, None, 0, 255, cv.NORM_MINMAX, dtype=cv.CV_8U # type: ignore[arg-type]
            )  # Normalize to 8-bit range (0-255)
        ...
        for k, v in ROIs.items():
            ...
            cv.putText(temp_color, v['label'], (x, y + 30), cv.FONT_HERSHEY_SIMPLEX, 1, color, 1)
